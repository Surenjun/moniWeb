...
<script>
import { updateAndroid } from '@/api/device'
export default {
  data() {
    return {
      carCode: '',
  ...
  mounted: function(argument) {
    if(this.androidInfo){
      this.carCode = this.androidInfo.carCode;
      this.deviceName = this.androidInfo.deviceName;
      this.valueSwitch = this.androidInfo.useStatus == 0? true:false
    }
  },
  methods: {
    handleCar() {
      var param = {
        ...
        id: this.androidInfo.id,
        carId: this.androidInfo.carId,
      ...
      updateAndroid(param).then(response => {
        this.$EventBus.$emit('updateAndroidSuccess');
        this.$message({
          type: 'success',
          message: '更新设备信息成功'
        });
