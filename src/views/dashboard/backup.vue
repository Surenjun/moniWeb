<template>
  <div class="app-container" :style="'background:repeat '+ditu+';background-size:100% 100%;'">
    ...
    <div id="main">
      <div class="stage border">
        ...
        <canvas id="canvasLine" width="640" height="960">您的浏览器不支持 画布。</canvas>
      </div>
      <div class="info font1">
        <div class="border" style="margin-bottom: 1em;padding: 1em;">
          <div class="row">
          ...
          <div class="row">
            <span class="color3">车&nbsp牌&nbsp号：</span><span style="width:180px;" class="color1">{{carNumber}}</span>
        ...
        <div style="display: flex;justify-content:space-between;">
          <div style="display: flex;flex-flow: column;flex-basis:400px;">
            <div class="back" style="margin-bottom: 1em;">
              <div class="row">
                <span class="color3">车&nbsp速：</span><span style="width:100px;" class="color1">{{animatedSpeed}}</span>
                <span class="color3">档&nbsp位：</span><span class="color1">{{gear}}</span>
              </div>
              <div class="row">
                <span class="color3">发动机转速：</span><span class="color1">{{animatedRp}}</span>
              </div>
            </div>
            <div class="back">
              <p class="font1 color3" style="text-align: center;">安全员行为</p>
              <div class="row">
                <p class="color3">【手势】</p>
                <div class="behavior">
  ...
  export default {
    ...
    data() {
      return {
        ...
        //info
        studentID: '320924198110056497',
        studentName: '爱丽丝',
        carType: 'C1',
        carNumber: '009学',
        startTime: '2019-05-21 00:00:00',
        examName: '通用评判',
        deduct: '',
        ...
        wsip: 'ws://192.168.7.189:9088',
        rtsp: 'rtsp://192.168.7.189:8554/exammonitor2',
        p: [
          {"x": 309.868, "y": 304.626, "score": 0.939421}, {
            "x": 324.98,
            "y": 350.654,
            "score": 0.854741
          }, {"x": 269.271, "y": 345.511, "score": 0.925079}, {
            "x": 233.826,
            "y": 421.459,
            "score": 0.912941
          }, {"x": 314.991, "y": 431.847, "score": 0.804031}, {
            "x": 375.822,
            "y": 360.559,
            "score": 0.857078
          }, {"x": 385.667, "y": 426.682, "score": 0.861115}, {
            "x": 355.466,
            "y": 472.206,
            "score": 0.594783
          }, {"x": 304.623, "y": 472.258, "score": 0.527813}, {"x": 269.304, "y": 472.221, "score": 0.525671}, {
            "x": 0,
            "y": 0,
            "score": 0
          }, {"x": 0, "y": 0, "score": 0}, {"x": 340.331, "y": 472.289, "score": 0.510017}, {
            "x": 381.019,
            "y": 472.287,
            "score": 0.121126
          }, {"x": 0, "y": 0, "score": 0}, {"x": 304.865, "y": 299.44, "score": 0.933476}, {
            "x": 330.236,
            "y": 299.681,
            "score": 0.929334
          }, {"x": 299.902, "y": 299.884, "score": 0.135383}, {"x": 350.359, "y": 304.898, "score": 0.901429}, {
            "x": 0,
            "y": 0,
            "score": 0
          }, {"x": 0, "y": 0, "score": 0}, {"x": 0, "y": 0, "score": 0}, {"x": 0, "y": 0, "score": 0}, {
            "x": 0,
            "y": 0,
            "score": 0
          }, {"x": 0, "y": 0, "score": 0}],
    ...
    methods:
      {
        init() { //开始
          ...
          this.initWebSocket()
          this.ctImg.clearRect(0, 0, 720, 1156)
        ...
        websocketonmessage(e) { //数据接收
          this.websockMsg = "接受数据长度：" + e.data.length
        ...
        play() { //开始播放
          ...
          this.message = '播放中' + Object.keys(this.axBuffer).length + '|' + Object.keys(this.wsBuffer).length

          let image = this.axBuffer[this.timestamp]
          let data = this.wsBuffer[this.timestamp]
        ...
        checkTimestamp() { //检测时间戳
          this.message = '缓冲数据' + Object.keys(this.axBuffer).length + '|' + Object.keys(this.wsBuffer).length
        ...
        refreshData(data) {  //刷新数据
          ...
          if(this.FL !== data.cheatTypeBean.leftHandGesture) this.FL = data.cheatTypeBean.leftHandGesture
          if(this.FR !== data.cheatTypeBean.rightHandGesture) this.FR = data.cheatTypeBean.rightHandGesture
          if(this.G !== data.cheatTypeBean.gesture) this.G = data.cheatTypeBean.gesture
          if(this.Vo !== data.cheatTypeBean.sound) this.Vo = data.cheatTypeBean.sound
          if(this.SC !== data.cheatTypeBean.fusha) this.SC = data.cheatTypeBean.fusha
          ...
          let last = data.deductionIDs.split(';').pop()
