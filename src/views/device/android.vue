<template>
  <div class="app-container">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list">
      ...
      <el-table-column label="设备编码" align="center" width="200">
        <template slot-scope="scope">
          <span>{{ scope.row.deviceCode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="绑定车辆" align="center" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.carCode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="考车号" align="center" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.carNo }}</span>
        </template>
      </el-table-column>
      <el-table-column label="使用状态" align="center" width="150">
      ...
      <el-table-column label="总运行内存" align="center" width="150">
        <template slot-scope="scope">
          <span>{{ scope.row.memoryTotal }}</span>
        </template>
      </el-table-column>
       <el-table-column label="运行内存使用量" align="center" width="150">
        <template slot-scope="scope">
          <span>{{ scope.row.memoryAvail }}</span>
        </template>
      </el-table-column>
      <el-table-column label="内置内存总量" align="center" width="150">
        <template slot-scope="scope">
          <span>{{ scope.row.interDiskTotal }}</span>
        </template>
      </el-table-column>
      <el-table-column label="内置内存使用量" align="center" width="150">
        <template slot-scope="scope">
          <span>{{ scope.row.interDiskAvail }}</span>
        </template>
      </el-table-column>
      <el-table-column label="外置内存总量" align="center" width="150">
        <template slot-scope="scope">
          <span>{{ scope.row.outerDiskTotal }}</span>
        </template>
      </el-table-column>
      <el-table-column label="外置内存使用量" align="center" width="150">
        <template slot-scope="scope">
          <span>{{ scope.row.outerDiskAvail }}</span>
        </template>
      </el-table-column>
      <el-table-column label="最后一次在线时间" align="center" width="200">
        <template slot-scope="scope">
          {{ scope.row.lastOnTime }}
        </template>
      </el-table-column>
      <el-table-column label="最后一次离线时间" align="center" width="200">
        <template slot-scope="scope">
...
<script>
import { getAndroidList,deleteAndroid,updateAndroid } from '@/api/device'
import waves from '@/directive/waves' // Waves directive
import Pagination from '@/components/Pagination' // Secondary package based on el-pagination
import PopAndroid  from '@/components/Device/PopAndroid' // Secondary package based on el-pagination

export default {
  name: 'device',
  // props:['idcard'],
  components: { Pagination,PopAndroid },
  directives: { waves },
  filters: {},
  data() {
    return {
  ...
  methods: {
    ...
    editCar() {
      ...
      this.$msgbox({
          ...
          message: h('p', null, [
            h('pop-android', {style: 'color: teal',props: {androidInfo: this.terminalInfo},key: new Date().getTime() })
          ]),
      ...
      this.$EventBus.$on('updateAndroidSuccess',()=>{
    ...
    deleteCar() {
      ...
      this.$confirm('确定删除吗?', '提示', {
        ...
        deleteAndroid(param).then(response => {
          if(response.code != 0){
    ...
    changeStatus(item) {
      ...
      updateAndroid(param).then(response => {
        this.getList();
        if (response.code !== 0) {
    ...
    getList() {
      ...
      getAndroidList(param).then(response => {
    ...
    destory() {
      this.$EventBus.$off('updateAndroidSuccess');
    }
  }
}
